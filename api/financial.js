...
// ════════════════════════════════════════════════════════════════════
// AFP · PLAN SEMANAL ACCIONABLE
// ════════════════════════════════════════════════════════════════════
//
// Construye una lista de COMPROMISOS verificables para la semana en curso,
// con nombre + razón explícita y monto exacto. La idea es darle al usuario
// instrucciones inmodificables que no le dejen espacio a improvisar.
//
// Modos adaptativos (auto-detectados, sin opción al usuario):
//   - survival   : saldo real < 50% gastos fijos del mes
//   - debt       : alguna deuda con interés >= 15% TAE
//   - build      : colchón Fiat < 100% de la meta
//   - accumulate : colchón listo y existen metas con deadline
//   - balance    : todo cubierto + superávit
//
// Output:
//   { week, mode, modeLabel, modeReason,
//     income, sacred, cushion, debts, goals, free,
//     realBalance, primaryAccountLabel }
async function afpWeekPlan(admin, userId, userMetadata = {}) {
  const today = new Date()
  ...
  const dow = today.getDay() || 7   // domingo=0→7
  ...
  // Saldo real de la cuenta primaria desde movimientos
  let realBalance = 0
  ...
  // ── INGRESOS ESPERADOS esta semana (heurística por frecuencia) ──
  // weekly: 1 ocurrencia ; biweekly: 1 si es semana par del mes
  const incomeItems = []
  incomes.forEach(i => {
    let amount = 0, reason = ''
    if (i.frequency === 'weekly') {
      amount = Number(i.amount); reason = 'ingreso semanal'
    } else if (i.frequency === 'biweekly') {
      // Aproximación: cada 2 semanas
      ...
      // 1 vez en la semana donde cae el "salary day" (default 1ro del mes) — simplificado: si lunes ≤ día 1 ≤ domingo
      const monthDay1 = new Date(today.getFullYear(), today.getMonth(), 1)
      if (monthDay1 >= monday && monthDay1 <= sunday) { amount = Number(i.amount); reason = 'ingreso mensual' }
    }
    if (amount > 0) {
      incomeItems.push({
        kind: 'income', label: i.name, amount, reason,
  ...
  fixed.filter(f => f.is_sacred).forEach(f => {
    ...
    sacredItems.push({
      kind: 'sacred', label: f.name, amount: Math.round(weekAmount),
      reason: 'compromiso sagrado · no se reduce nunca',
  ...
  const realOrFallback = realBalance || monthlyIncome  // si no hay primary, usa ingreso mes

  if (realOrFallback < monthlyFixed * 0.5) {
    mode = 'survival'; modeLabel = 'Supervivencia'
    modeReason = `Tu saldo real ($${Math.round(realOrFallback).toLocaleString()}) no alcanza ni la mitad de tus gastos fijos del mes ($${Math.round(monthlyFixed).toLocaleString()}). Solo dispersa sagrado + lo crítico.`
  } else if (highInterestDebt) {
    mode = 'debt'; modeLabel = 'Liquidación de deuda'
    modeReason = `Tienes deuda al ${Number(highInterestDebt.interest_rate).toFixed(0)}% TAE ("${highInterestDebt.name}"). Cada peso a ese saldo es la mejor inversión posible.`
  } else if (cushionProgress < 100) {
    mode = 'build'; modeLabel = 'Construcción de colchón'
    modeReason = `Tu colchón cubre ${cushionProgress.toFixed(0)}% de la meta (${cushion.target_months} meses de gastos fijos). Prioridad: terminar de armarlo.`
  } else if (goals.length > 0) {
    mode = 'accumulate'; modeLabel = 'Acumulación a metas'
    modeReason = `Colchón listo. Toca atacar tus metas con plazo.`
  }

  // ── COLCHÓN: aporte semanal solo si estamos en modo build ──
  const cushionItems = []
  if (mode === 'build' || mode === 'survival') {
    const faltante = Math.max(0, cushionTarget - Number(cushion.current_balance))
    // Reparte el faltante en hasta 12 semanas
    const weeklyContrib = Math.min(faltante, Math.round(monthlyFixed / 4 / 2))   // tope: medio gasto-fijo-semana
    if (weeklyContrib > 0) {
      cushionItems.push({
        kind: 'cushion', label: 'Colchón Fiat',
  ...
  // ── METAS con deadline: prorrateo por semanas restantes ──
  const goalItems = []
  goals.forEach(g => {
    ...
    const weeklyShare = Math.round(remaining / weeksLeft)
    goalItems.push({
      kind: 'goal', label: g.name, amount: weeklyShare,
      reason: `meta $${Math.round(g.target_amount).toLocaleString()} en ${weeksLeft} semanas · faltan $${Math.round(remaining).toLocaleString()}`,
      deadline: g.deadline,
    })
  })

  // ── FREE: lo que queda después de TODO lo comprometido ──
  ...
  // Si la dispersión deja menos del mínimo de vivir, reducimos metas (no sagrado)
  const warnings = []
  if (freeAmount < minLivingWeekly && goalItems.length > 0) {
    const deficit = minLivingWeekly - freeAmount
    // Reduce metas proporcional
    ...
    if (goalTotal > 0) {
      ...
      warnings.push(`Reducimos $${Math.round(deficit).toLocaleString()} de aporte a metas para respetar tu mínimo semanal de vivir ($${minLivingWeekly.toLocaleString()}).`)
    }
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'afp_weekplan_get') {
      const result = await afpWeekPlan(admin, userId, user.user_metadata || {})
